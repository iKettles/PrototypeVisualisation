...
function calculateTimeThreshold(threshold, timestamp) {
  var timeThreshold = null;
  $.each(threshold, function(key, time) {
    var hours = new Date(timestamp).getUTCHours();
    if(key !== 'night') {
      if(hours >= thresholds.time[key].start && hours < thresholds.time[key].end) {
        timeThreshold = key;
        return false;
      }
    } else {
...
function calculateThreshold(threshold, value) {
  var batch = null;
  $.each(threshold, function(key, num) {
    if(value >= num) {
      batch = key;
    }
  });
